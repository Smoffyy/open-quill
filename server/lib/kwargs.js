...
export const KWARG_CONTROLS = ['auto', 'toggle', 'slider', 'select'];
...
export function resolveKwargValues(defs, requested, isAdmin = false, useFirstValue = false) {
  ...
  for (const d of defs) {
    ...
    let v = useFirstValue ? (d.values[0] ?? defaultValueOf(d)) : defaultValueOf(d);
    if (!useFirstValue && d.visible && (isAdmin || !d.adminOnly)) {
      const asked = req[d.id];
      if (asked != null && d.values.includes(String(asked))) v = String(asked);
    }
    out[d.id] = v === '' ? null : v;
  }
...
export function kwargPayload(defs, values) {
  ...
  for (const d of defs) {
    ...
    if (!d.parentId && !d.visible && !d.sendWhenHidden) continue;
...
export function publicKwargDefs(model) {
  return kwargDefs(model).map(d => ({
    ...
    parentId: d.parentId, rules: d.rules
  }));
}
