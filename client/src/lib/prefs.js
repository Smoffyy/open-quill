...
const REOPEN_SETTINGS_KEY = 'oq-reopen-settings';

export function reopenSettingsAfterReload(tab) {
  try { sessionStorage.setItem(REOPEN_SETTINGS_KEY, tab); } catch {}
}

export function takeSettingsToReopen() {
  try {
    const tab = sessionStorage.getItem(REOPEN_SETTINGS_KEY);
