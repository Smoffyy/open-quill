...
import { Search, X, ChevDown, Chevron, Plus, Trash, Info, Wand, FileText, Upload, Eye, CodeTag, DotsV, Chat } from '../ui/icons.jsx';
...
function Directory({ installed, onClose, onInstall }) {
  ...
  return createPortal(
    <Dialog className="sk-dir" overlayClassName="sk-dir-overlay" labelledBy="sk-dir-title" onClose={onClose}>
      <div className="sk-dir-head">
        <h2 className="sk-dir-title" id="sk-dir-title">{t('Directory')}</h2>
        <CloseButton plain className="sk-icon-btn" onClick={onClose} />
      </div>
...
export default function SkillsSection({ onTrySkill, browse = false }) {
  ...
  const [dir, setDir] = useState(browse);
