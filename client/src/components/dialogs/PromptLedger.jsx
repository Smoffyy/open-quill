...
function Section({ label, role, tokens, text, open, onToggle, color }) {
  return (
    <div className="pl-seg">
      <button type="button" className="pl-head" aria-expanded={open} onClick={onToggle}>
...
export default function PromptLedger({ chatId, modelId, onClose }) {
  ...
  useEffect(() => {
    ...
    return () => { live = false; clearTimeout(copyTimer.current); };
  }, [chatId, modelId]);
  ...
  return (
    <Dialog className="modal pl-modal" label={t('What gets sent')} onClose={onClose} focusSelf>
