    ...
    <>
      ...
      <Card title={t("Interface preset")} sub={t("Switches the entire UI between the two looks instantly, for every connected client.")}>
        <div className="field">
          <SegPick value={cfg.uiPreset || 'anthropic'} options={[['anthropic', tk('Anthropic')], ['openai', tk('OpenAI')]]}
            onChange={(v) => setCfg(c => ({ ...c, uiPreset: v, appFont: v === 'openai' ? 'sans' : 'newsreader' }))} />
      ...
      <Card title={t("Typography & footer")} sub={t("The overall voice of the interface.")}>
        <div className="field"><label>{t("Interface font")}</label>
          <SegPick value={cfg.appFont || 'newsreader'} options={[['newsreader', tk('Newsreader (default)')], ['sourceserif', tk('Source Serif')], ['sans', tk('Open Sans')]]}
            onChange={(v) => setCfg(c => ({ ...c, appFont: v }))} />
          <div className="muted-note">{t("The display font used for headings, greetings, and assistant text across the entire UI. Newsreader is the closest open match to the Anthropic serif. Open Sans gives a cleaner, sans-serif look everywhere.")}</div>
        </div>
