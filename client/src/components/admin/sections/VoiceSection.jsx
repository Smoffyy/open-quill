    ...
    <>
      ...
      {(settings.voiceMicEnabled || settings.voiceCallEnabled) && (
        <Card title={t("Speech-to-text")} sub={t("How spoken audio becomes text.")}>
          <div className="field"><label>{t("Engine")}</label>
            <SegPick value={settings.voiceSttEngine || 'browser'} options={[['browser', tk('Browser built-in')], ['server', tk('Server (Whisper)')]]}
          ...
          {settings.voiceSttEngine === 'server' && <>
            <div className="field"><label>{t("Base URL")}</label>
              ...
              <div className="muted-note">{t("The server calls {base}/audio/transcriptions on your behalf. Keys never reach the browser.")}</div>
            </div>
            <div className="two-col">
              <div className="field"><label>{t("API key")} <span className="muted-note" style={{ display: 'inline' }}>{t("(optional for local)")}</span></label>
      ...
      {settings.voiceCallEnabled && (
        <Card title={t("Text-to-speech")} sub={t("How replies are read aloud during calls.")}>
          <div className="field"><label>{t("Engine")}</label>
            <SegPick value={settings.voiceTtsEngine || 'browser'} options={[['browser', tk('Browser built-in')], ['server', tk('Server (OpenAI-compatible)')]]}
              onChange={(v) => setSettings(s => ({ ...s, voiceTtsEngine: v }))} />
            <div className="muted-note">{t("Browser uses the voices Chrome exposes, with no setup. A local system voice is preferred automatically; Chrome network voices are synthesised by Google, so they need internet. Server sends text to any OpenAI-compatible /audio/speech endpoint: openedai-speech, Kokoro-FastAPI, Piper wrappers, or OpenAI.")}</div>
          </div>
          ...
          <div className="two-col">
            <div className="field"><label>{t("Voice")}</label>
              ...
              <div className="muted-note">{settings.voiceTtsEngine === 'server' ? t('The voice name sent to the endpoint (e.g. alloy, af_bella for Kokoro).') : 'Matched against the browser\u2019s installed voice names. Leave blank for the system default.'}</div>
            </div>
