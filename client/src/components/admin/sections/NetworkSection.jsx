...
export default function NetworkSection() {
  ...
  useEffect(() => {
    ...
    // Polling stops while the tab is hidden: a background admin panel should not
    // keep asking the server for a log nobody is reading.
  ...
  async function clearLog() {
    try { await api.del('/api/admin/egress-log'); setLog({ entries: [], allowed: 0, blocked: 0 }); } catch {}
  }
  ...
  return (
    <>
      <Card title={t('Outbound policy')}
        ...
        <Rows>
          <ToggleRow label={t('Block public internet')} on={blocking}
          ...
          <ToggleRow label={t('Exempt web search')} on={config.egressAllowWebSearch !== false}
      ...
      <Card title={t('Browser policy')}>
        <Rows>
          <ToggleRow label={t('Serve every asset from this origin')} on={config.localOnly !== false}
      ...
      <Card title={t('Connection log')}
        sub={t('Every host this server has tried to reach since it started. Held in memory, so it empties on restart.')}
        actions={entries.length ? <Btn size="sm" kind="danger" onClick={clearLog}>{t('Clear log')}</Btn> : null}>
        <Stats items={[
            ...
            <Table head={[
              ...
              {entries.map((e, i) => (
                <tr key={e.host + i}>
                  ...
                  <td className="dim">{fmtAgo(e.last)}</td>
