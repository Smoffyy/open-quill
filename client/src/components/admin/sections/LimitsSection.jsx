  ...
  return (
    <>
      ...
      <Card title="Request queue" sub="Keep small local servers from thrashing between models.">
        <SettingRow last label="Model queue" note="Only one model runs at a time. Requests for the same model run together; a request for a different model waits until the current one finishes, instead of swapping it out mid-response."
      ...
      <Card title="Usage budgets" sub="Monthly spend caps based on per-model pricing. Per-member overrides live in the Members section. Set 0 for no limit.">
        ...
        <SettingRow last label="Enforce budget" note="When on, users at or over their cap cannot send new messages until next month. When off, the banner is informational only. Admins are never blocked."
