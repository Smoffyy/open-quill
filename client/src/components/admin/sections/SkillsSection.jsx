  ...
  return (
    <>
      ...
      {edit && (
        <div className="fn-editor">
          <div className="field"><label>Skill name</label>
            <input value={edit.name} onChange={(e) => setEdit(x => ({ ...x, name: e.target.value }))} placeholder="brand-voice" />
            <div className="muted-note">Lowercase letters, digits, hyphens. This is the name the model loads.</div>
          </div>
          <div className="field"><label>Description</label>
            <input value={edit.description} onChange={(e) => setEdit(x => ({ ...x, description: e.target.value }))} placeholder="How to write copy in our brand voice. Load before writing any marketing text." />
      ...
      <div className="fn-list">
        {skills.length === 0 && !edit && <div className="muted-note">No skills yet.</div>}
        {skills.map(sk => (
          <div key={sk.id} className="fn-card">
            ...
            <div className="fn-card-actions">
              <div className={'switch' + (sk.enabled ? ' on' : '')} title="Enabled" onClick={() => toggle(sk)} />
