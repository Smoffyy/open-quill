...
**Every failure teaches.** A missing argument names the argument and says what it is for; an unknown tool suggests the nearest real name (edit distance) and lists the valid ones; a blocked path explains the boundary and gives a correct example; a not-found command lists what *is* installed. `turn.js` already appends a "this identical call failed N times" note. The goal is that a small model's second attempt succeeds, so when adding a tool or a guard, write the error for the model that just got it wrong.
